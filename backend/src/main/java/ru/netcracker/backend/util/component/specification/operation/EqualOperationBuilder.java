...
public class EqualOperationBuilder extends OperationBuilder {
    ...
    public Predicate build() {
        switch (getFilter().getProperty()) {
            case "subscriber":
                return equals(getRoot().join(Auction_.subscribedUsers).get(User_.username), getFilter().getValue());
            case "creator":
                return equals(getRoot().join(Auction_.creator).get(User_.username), getFilter().getValue());
            default:
